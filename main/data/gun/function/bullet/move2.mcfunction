...
execute unless entity @s[tag=Tag.IsInShooter] if entity @e[dx=0,tag=!Entity.Bullet,type=!experience_orb,type=!item,nbt=!{Invulnerable:1b},sort=nearest,limit=1] as @e[dx=0,tag=!Entity.Bullet,type=!experience_orb,type=!item,nbt=!{Invulnerable:1b},sort=nearest,limit=1] if data entity @s Health positioned ~-0.8 ~-0.8 ~-0.8 if entity @s[dx=0] unless score @s player.uid = @e[type=marker,tag=Entity.Bullet,sort=nearest,limit=1] gun.bullet_owner positioned ~0.8 ~0.8 ~0.8 run function gun:bullet/hit/hit

execute unless block ^ ^ ^0.1 #main:no_collision run function gun:bullet/kill
