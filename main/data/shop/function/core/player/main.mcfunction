execute if entity @n[tag=Entity.ShopOwner,distance=..5] if predicate shop:core/player/looking_at_shop_owner/shop_owner unless entity @s[tag=Event.LookingAtShopOwner] run function shop:core/player/looking_at_shop_owner/init

execute if entity @s[tag=Event.LookingAtShopOwner] unless entity @n[tag=Entity.ShopOwner,distance=..5] run function shop:core/player/reset_looking_at
